...
from .models import Block, Process
...
class Memory:
    ...
    def allocate(self, process: Process, method: str) -> bool:
        ...
        if hole.size == process.size:
            ...
            used = Block(start=hole.start,
                         size=process.size,
                         free=False,
                         tag=process.name)
            rest = Block(start=hole.start + process.size,
                         size=hole.size - process.size,
                         free=True)
    ...
    def compaction_cost(self) -> int:
        """Total bytes moved across all compaction calls."""
